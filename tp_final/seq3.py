...
class SampleAccurateSequencer:
    def __init__(self, samples: List[Sample], bpm=BPM, steps=STEPS, tracks=TRACKS,
        ...
        for i in range(self.tracks_n):
            ...
            pat = Pattern.random(self.steps, max(1, len(self.samples)), rng=track_rng)
            self.tracks.append(Track(i, TRACK_TO_CHANNEL[i % channels_out], pat, track_rng))
    ...
    def _regenerate_patterns(self):
        ...
        # Use each track's RNG to create patterns
        for track in self.tracks:
            track.pattern = Pattern.random(self.steps, max(1, len(self.samples)), rng=track.rng)
        self.loop_target = random.randint(REPEAT_MIN, REPEAT_MAX)
        # reset step timing to align with next boundary (no change to sample_pos)
        # keep next_step_sample as-is; next triggers will continue naturally
        print(f"[callback] Regenerated patterns. Next repeat target: {self.loop_target}")
